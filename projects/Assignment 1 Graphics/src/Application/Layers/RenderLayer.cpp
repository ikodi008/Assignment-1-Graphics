...
RenderLayer::RenderLayer() :
	...
	_renderFlags(RenderFlags::EnableColorCorrection&RenderFlags::EnableColdCorrection&RenderFlags::EnableBWCorrection),
...
void RenderLayer::OnRender(const Framebuffer::Sptr& prevLayer)
{
	...
	{
		LUTS = app.CurrentScene()->GetColorLUT(0);
	}
	else if (_renderFlags == RenderFlags::EnableColdCorrection)
	{
		LUTS = app.CurrentScene()->GetColorLUT(1);
	}
	else if (_renderFlags == RenderFlags::EnableBWCorrection)
	{
		LUTS = app.CurrentScene()->GetColorLUT(2);
	}
	if (LUTS)
	{
		LUTS->Bind(14);
