...
void main() {
	...
	float SpecularStrength = texture(u_Material.Specular, inUV).r;
	...
	vec3 lightAccumulation = CalcAllLightContribution(inWorldPos, normal, u_CamPos.xyz, SpecularStrength);
	...
	frag_color = vec4(ColorCorrect(mix(result, reflected, SpecularStrength)), textureColor.a);
}