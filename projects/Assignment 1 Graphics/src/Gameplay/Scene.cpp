...
namespace Gameplay {
	...
	void Scene::SetColorLUT(const Texture3D::Sptr& texture, int nummode) {
		if (nummode == 0)
		{
		...
		{
			_colorColdCorrection = texture;
		}
		else if (nummode == 2)
		{
			_colorBWCorrection = texture;
		}
	}

	const Texture3D::Sptr& Scene::GetColorLUT(int nummode) const {
		if (nummode == 0)
		{
		...
		{
			return _colorColdCorrection;
		}
		else if (nummode == 2)
		{
			return _colorBWCorrection;
		}
	}
